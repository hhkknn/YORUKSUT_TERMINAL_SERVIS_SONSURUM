    ...
    public class AddOrUpdateIade
    {
        public Response addOrUpdateIade(string dbName, int kullaniciId, Iade iade)
        {
            ...
            try
            {
                ...
                oDocuments.CardCode = iade.CardCode;
                oDocuments.NumAtCard = iade.WayBillNo;

                DateTime dt = new DateTime(Convert.ToInt32(iade.TaxDate.Substring(0, 4)), Convert.ToInt32(iade.TaxDate.Substring(4, 2)), Convert.ToInt32(iade.TaxDate.Substring(6, 2)));

                oDocuments.TaxDate = dt;
                oDocuments.Comments = iade.Comments;

                oDocuments.ShipToCode = iade.ShipToCode;

                //oDocuments.BPL_IDAssignedToInvoice = 1;??
                int i = 0;
                foreach (var item in iade.iadeDetays)
                {
                    ...
                    if (item.LineNum != null)
                    {
                        ...
                        oDocuments.Lines.BaseType = 234000031;
                    ...
                    foreach (var item_BinCode in item.iadeDetayParti.Where(x => x.DepoYeriId != null && x.DepoYeriId != ""))
                    {
                    ...
                    foreach (var aifteam in item.iadeDetayParti)
                    {
                ...
                oRS.DoQuery("Select BPLid from OWHS where WhsCode = '" + iade.iadeDetays[0].WareHouse + "' "); //D21100//Eğer SAP şubeli bir yapıda ise ona göre işlem yapılıyor. owhs tablosu Şube tablosudur. Kayıt varsa şubeli yapıda olduğunu gösterir.
